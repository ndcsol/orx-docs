...
- `itinerary_price`: ([Itinerary Price Information](/versions/v2/glossary#itinerary-price-information)) An object containing all the information about the pricing of the order.
- `itinerary`: (`Array<`[Order Flight Fare](/versions/v2/glossary#order-flight-fare)`>`) An array containing the details of each bound's Order Flight Fare.
- `session`: ([Session](/versions/v2/glossary#session)) Information about the long-lived booking session.  
...
- `booking_references`: (`Array<Record>`) An array containing booking reference information. There may or may not be one locator per marketing airline in the booking, depending on the type of the partnership.
  - `booking_references[].airline`: ([Airline IATA Code](/versions/v2/glossary#airline-iata-code)) The IATA code of the airline.
  - `booking_references[].locator`: (`string`) The unique identifier for the booking reference provided by the airline.
- `itinerary_price`: ([Itinerary Price Information](/versions/v2/glossary#itinerary-price-information)) An object containing all the information about the pricing of the order.
- `itinerary`: (`Array<`[Order Flight Fare](/versions/v2/glossary#order-flight-fare)`>`) An array containing the details of each bound's Order Flight Fare.
- `passengers`: (`Array<`[Flight Order Passenger Information](/versions/v2/glossary#flight-order-passenger-information).`>`) An array containing the details of all passengers.
- `seats`: (`Array<`[Order Seat-Like](/versions/v2/glossary#order-seat-like)`>`) An array of all the seats within the order.
