...
You may provide a new payment or an alternative payment method here; or, create one using the [Payment Management](/versions/v2/payment) endpoint.
...
If paying with a regular payment object and have not created a payment object using the [Payment Management](/versions/v2/payment#update-payment) endpoint, you must provide the payment information in this request. 
...
- `payment`: ([Payment Information](/versions/v2/glossary#payment-information)?) Payment to be used for the order.

#### Order using an Alternative Payment

Alternatively, if you are planning to use an alternative payment method; and, [have not attached an alternative payment to the session](/versions/v2/payment#update-alternative-payment), you can provide the alternative payment information in this request.
...
- `alternativePayment`: ([Alternative Payment Information](/versions/v2/glossary#alternative-payment-information)?) Alternative Payment to be used for the order.
...
The response will be a new [session](/versions/v2/session#session-object). The order session serves as a reference to manage and access specific orders.

> 🧨 Once the order is placed, **the search session that you have been using will be invalidated**. For any action from here on, you will need to use the new [public](/versions/v2/glossary#public-id) returned in this response.

> 🛎 The session returned will be of flow `FLIGHT_BOOKING`. This session is a long-lived one that will expire after the travel date of the last flight in the itinerary. You can see the details of the session using the [Get Session](/versions/v2/session#accessing-session-information) endpoint.
