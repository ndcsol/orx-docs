...
The response contains an array of [Flight Fares](/versions/vv3glossary#flight-fare), each representing of set of segments fulfilling the bound at hand.
...
| ----------- | ----------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------------- |
| 404         | Session not found or expired                    | Request targeting a non-existent session.                           | [ORX_SESSION_NOT_FOUND](/versions/v3/errors#orx-errors) |
| 409         | Bound [bound] is not available for this session | Request is attempting to retrieve flights for a non-existent bound. | [ORX_INVALID_BOUND](/versions/v3/errors#orx-errors)     |
| 412         | Not all flights are selected                    | The Itinerary has not yet been fully selected.                      | [ORX_UNORDERED_FLIGHTS](/versions/v3/errors#orx-errors) |
...
The response will be of type [Itinerary Flight Fare](/versions/v3/glossary#itinerary-flight-fare), a Flight Fare only containing the selected Brand Information.
...
| ----------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| 404         | Session not found or expired                                                       | Request targeting a non-existent session.                                                                        | [ORX_SESSION_NOT_FOUND](/versions/v3/errors#orx-errors)    |
| 400         | [Validation Error]                                                                 | Failed Validation                                                                                                | [ORX_VALIDATION_EXCEPTION](/versions/v3/errors#orx-errors) |
| 412         | No flight selected for bound [bound] Flights can only be selected in order         | Request is selecting fares for a bound without having selected the fare for its preceding bound(s).              | [ORX_UNORDERED_FLIGHTS](/versions/v3/errors#orx-errors)    |
| 404         | Flight brand not found or expired                                                  | Selected flight brand is not found.                                                                              | [ORX_BRAND_NOT_FOUND](/versions/v3/errors#orx-errors)      |
| 404         | Could not find fare                                                                | Selected flight fare is not found.                                                                               | [ORX_FARE_NOT_FOUND](/versions/v3/errors#orx-errors)       |
| 409         | The fare requested does not belong to bound [bound]                                | Selected flight fare does not belong to the bound it is selected for.                                            | [ORX_INVALID_BOUND](/versions/v3/errors#orx-errors)        |
| 409         | The brand and the fare provided are not compatible                                 | The selected brand does not belong to the selected fare.                                                         | [ORX_BRAND_FARE_MISMATCH](/versions/v3/errors#orx-errors)  |
| 409         | The fare or the brand provided are not compatible selections of the previous bound | The brand and/or the fare cannot be combined with the brand and/or the fares that have previously been selected. | [ORX_BRAND_FARE_MISMATCH](/versions/v3/errors#orx-errors)  |
| 404         | Seat not found or expired                                                          | The request is attempt to select non-existent seats.                                                             | [ORX_SEAT_NOT_FOUND](/versions/v3/errors#orx-errors)       |
| 404         | Passenger not found or expired                                                     | The request is attempt to select non-existent seats.                                                             | [ORX_PASSENGER_NOT_FOUND](/versions/v3/errors#orx-errors)  |
...
> 🛎 This endpoint is only intended for active search [sessions](/versions/v3/session#session-object). Requesting the itinerary for a session of type FLIGHT_BOOKING will result in a 400 error.

### Response

The response will be an array of type [Itinerary Flight Fare](/versions/v3/glossary#itinerary-flight-fare), representing the selected flights in order.

### Possible Errors

| Status Code | Error                                                         | Reason                                            | Reason Code                                             |
| ----------- | ------------------------------------------------------------- | ------------------------------------------------- | ------------------------------------------------------- |
| 400         | Retrieve itinerary is only allowed for active search sessions | The session in the request is not of type FLIGHT. | [ORX_SESSION_NOT_FOUND](/versions/v3/errors#orx-errors) |
| 404         | Session not found or expired                                  | Request targeting a non-existent session.         | [ORX_SESSION_NOT_FOUND](/versions/v3/errors#orx-errors) |

# Forward-Looking Selection

We realize that the [Flight Selection](/versions/v3/flights/selection) process can be a bit cumbersome, especially when trying to progressively compute the different possibilities when selecting certain brands and fares. To make this process easier, we've created the Forward Looking Selection endpoints. These endpoints are designed to help you create more advanced selection flows, where you can progressively receive more granular information about the itinerary that is being selected.

## Estimate Commission

`POST /flight/[sessionId]/bound/[boundIndex]/estimate-commission`

Use this endpoint to estimate the possible the hightest and the lowest commission that can be earned from selecting a fare brand in a specific bound. This endpoint requires the [Public ID](/versions/v3/glossary#public-id) of the flight fares that had been selected in the previous steps. You may optionally include the [Public ID](/versions/v3/glossary#public-id) of the brands that had been selected in the previous steps.
...
- `currentBrands`: (`Array<`[Public ID](/versions/v3/glossary#public-id)`>`) The public ids of the brands you want to receive the commission estimate for.
- `selectedFare`: (`Array<`[Public ID](/versions/v3/glossary#public-id)`>`) The public ids of the fares selected in the previous bounds, in order (from the first bound to the current bound.)
- `selectedBrands`: (`Array<`[Public ID](/versions/v3/glossary#public-id)`>?`) The public ids of the bounds selected in the previous bounds, in order. Each of selected brands is associated with the fare of the same index in the `selectedFares` array.
...
> 🧨 The endpoint currently only supports one-way and round-trip itineraries. Multi-city itineraries are not supported.

> 🧨 There is a limit of 10 brands that can be passed in the `currentBrands` array.

- `brands`: (`Array<Record>`) The pricing and commission estimates for the brands that were passed as `currentBrands`.
  - `brands[].public` : ([Public ID](/versions/v3/glossary#public-id)) The public id of the current brand corresponding to the commission estimate.
...
| 400         | [Validation Error]                                              | Failed Validation                                                                                                              | [ORX_VALIDATION_EXCEPTION](/versions/v3/errors#orx-errors)  |
| 404         | Session not found or expired                                    | Request targeting a non-existent. session.                                                                                     | [ORX_SESSION_NOT_FOUND](/versions/v3/errors#orx-errors)     |
| 400         | Invalid bound                                                   | The bound that the request is asking for is exceeding the number of bounds in this session.                                    | [ORX_INVALID_BOUND](/versions/v3/errors#orx-errors)         |
| 400         | Selected fares or bounds are not applicable for the first bound | The bound index is 0, but the `selectedFare` and/or the `selectedBrand` are not empty.                                         | [ORX_BOUND_ENTITY_MISMATCH](/versions/v3/errors#orx-errors) |
| 400         | Number of selected entities must correspond to the bound index  | The number of items in the `selectedFare` or `selectedBrand` arrays are not corresponding to the bound the in the request URL. | [ORX_BOUND_ENTITY_MISMATCH](/versions/v3/errors#orx-errors) |
| 400         | Selected entities must represent consecutive bounds.            | The items in the `selectedFare` array are not corresponding to consecutive bounds, in order.                                   | [ORX_BOUND_ENTITY_MISMATCH](/versions/v3/errors#orx-errors) |
...
- `fares`: (`Array<`[Fare Matching](/versions/v3/glossary#fare-matching)`>`) The array representing the fares that could be matched with the selected fare(s) in the immediate next bound.
