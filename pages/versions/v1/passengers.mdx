...
`PUT /versions/v1/passenger/[session_id]`
...
> 🛎 You should use the `public` property to identify the passenger in the following endpoints.

## Retrieving a Single Passenger

`GET /versions/v1/passenger/[session_id]/[passenger_public]`
...
`PUT /versions/v1/passenger/[session_id]/[passenger_public]`
...
- `phone_number`: Passenger's phone number in E.164 format

- `email`: Passenger's email

- `personal_info`: Passenger's personal information

  - `personal_info.date_of_birth`: Passenger's date of birth [ISO8601](https://www.iso.org/iso-8601-date-and-time-format.html) date format.

  - `personal_info.first_name`: Passenger's first name

  - `personal_info.last_name`: Passenger's last name

  - `personal_info.middle_name`: (optional) Passenger's middle name

  - `personal_info.gender`: Passenger's gender, must be one of M, F, X

- `frequent_flyer`: (optional) Passenger's frequent flyer information

  - `frequent_flyer.program_id`: Passenger's frequent flyer program id, must be a IATA airline code.

  - `frequent_flyer.id`: Passenger's frequent flyer id.
