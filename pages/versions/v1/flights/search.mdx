...
- `passengers`: An array of objects that represents the number and type of passengers. The example above includes one adult passenger.

  - Supported passenger codes are `ADT` and `CHD`.

- `preferences`: An object that represents additional search preferences. It could be used to specify a suppression method, discount codes, providers, and PCCs. See [advanced usage](#advanced-usage) for more info.
...
```json copy
{
  ...
  "passengers": [
    //versions/v1/passengers
  ],
  "preferences": {
    "suppression": "NO_SUPPRESSION",
    "discount_codes": ["discount"],
    "providers": ["ATPCO", "FLX", "SABRE", "FNDC"],
    "pcc": {
      "ATPCO": "4R5L",
      "FLX": "56NM",
      "SABRE": "B5E2",
...
The `discount_codes` property can accept an array or a single string.
...
The `providers` controls which providers are queried. The possible providers are:

- `ATPCO` `FLX` `SABRE` `FNDC`
...
The `pcc` property accepts an object, where keys are providers and values are the relevant PCCs for those providers.
