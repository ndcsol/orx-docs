...
The response contains an array of [Flight Fares](/versions/v1/glossary#flight-fare), each representing of set of segments fulfilling the bound at hand.
...
The response will be of type [Itinerary Flight Fare](/versions/v1/glossary#itinerary-flight-fare), a Flight Fare only containing the selected Brand Information.
...
> 🛎 This endpoint is only intended for active search [sessions](/versions/v1/session#session-object). Requesting the itinerary for a session of type FLIGHT_BOOKING will result in a 400 error.

### Response

The response will be an array of type [Itinerary Flight Fare](/versions/v1/glossary#itinerary-flight-fare), representing the selected flights in order.
