...
- `total_tax` : (`number`) The total amount of tax in the context of its parent.
...
- `total_surcharge` : (`number`) The total amount of surcharges in the context of its parent.
...
A flight fare is a representation of a bound. It may contain one or more [segments](#segment-information). It will also contain different [Brands](/versions/v1/glossary#flight-brand-information), one of which will have to be selected.

### Sub Properties

- `public`: ([Public ID](/versions/v1/glossary#public-id)) The public ID of the flight fare. This will be used to select and retrieve the flight.
...
- `airports`: (`Array<`[Flight Airport Information](/versions/v1/glossary#flight-airport-information)`>`) An array representing all of the airports visited.
...
An **itinerary** flight fare is a representation of a bound that has **already been selected**. It looks just like the [Flight Fare](#flight-fare), but it only has a single [Brand Information](/versions/v1/glossary#flight-brand-information); the one that has been selected.

### Sub Properties

It contains the same properties as a [Flight Fare](/versions/v1/glossary#flight-fare), with the exception of its `brand_information`:

- `brand_information`: ([Brand Information](/versions/v1/glossary#flight-brand-information)) The object containing the selected brand information.
...
- `price`: (`Record?`) The price of the seat.
  ...
  - `base_price`: (`number?`) The base price of the seat.
...
- `frequent_flyer`: (`Record?`) Passenger's frequent flyer information.
  - `frequent_flyer.program_id`: (`string`) Passenger's frequent flyer program id, must be a IATA airline code.
  - `frequent_flyer.id`: (`string`) Passenger's frequent flyer id.
