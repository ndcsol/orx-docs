...
The response contains an array of [Flight Fares](/versions/vv4glossary#flight-fare), each representing of set of segments fulfilling the bound at hand.
...
The response will be of type [Itinerary Flight Fare](/versions/v4/glossary#itinerary-flight-fare), a Flight Fare only containing the selected Brand Information.
...
> 🛎 This endpoint is only intended for active search [sessions](/versions/v4/session#session-object). Requesting the itinerary for a session of type FLIGHT_BOOKING will result in a 400 error.

### Response

The response will be an array of type [Itinerary Flight Fare](/versions/v4/glossary#itinerary-flight-fare), representing the selected flights in order.
...
- `currentBrands`: (`Array<`[Public ID](/versions/v4/glossary#public-id)`>`) The public ids of the brands you want to receive the commission estimate for.
- `selectedFare`: (`Array<`[Public ID](/versions/v4/glossary#public-id)`>`) The public ids of the fares selected in the previous bounds, in order (from the first bound to the current bound.)
- `selectedBrands`: (`Array<`[Public ID](/versions/v4/glossary#public-id)`>?`) The public ids of the bounds selected in the previous bounds, in order. Each of selected brands is associated with the fare of the same index in the `selectedFares` array.
...
- `brands`: (`Array<Record>`) The pricing and commission estimates for the brands that were passed as `currentBrands`.
  - `brands[].public` : ([Public ID](/versions/v4/glossary#public-id)) The public id of the current brand corresponding to the commission estimate.
...
- `fares`: (`Array<`[Fare Matching](/versions/v4/glossary#fare-matching)`>`) The array representing the fares that could be matched with the selected fare(s) in the immediate next bound.
