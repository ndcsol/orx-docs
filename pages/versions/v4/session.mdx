...
```
- `session`: ([Session](/versions/v4/glossary#session)) the session object describing the expiry, the creation time and the flow of the session.  
...
> ⛳ By default, session expirations are set to be the same as the shortest-lived offer in the response, but you can request the API set the session expiration to a static 2-hour window by setting the header `Prefer: dynamic-session-expiry=false`. This is not recommended for production use, as certain offers may expire before the session does.
