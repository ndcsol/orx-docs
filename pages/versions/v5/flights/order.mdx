...
The response will be a new [session](/versions/v5/session#session-object). The order session serves as a reference to manage and access specific orders.

> 🧨 Once the order is placed, **the search session that you have been using will be invalidated**. For any action from here on, you will need to use the new [public](/versions/v5/glossary#public-id) returned in this response.

> 🛎 The session returned will be of flow `FLIGHT_BOOKING`. This session is a long-lived one that will expire after the travel date of the last flight in the itinerary. You can see the details of the session using the [Get Session](/versions/v5/session#accessing-session-information) endpoint.
...
| 412 | Pricing not found. Please price the session before attempting to order | Pricing call hasn’t been made for the current combination of fares, brands and seats. | ORX_PRICING_NOT_FOUND |
...
| 412 | Not all flights are selected | The Itinerary has not yet been fully selected. | ORX_UNORDERED_FLIGHTS |
