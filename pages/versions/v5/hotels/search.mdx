...
> ⛳ Hotel sessions have a default 2-hour expiration time and use the `HOTEL` flow type to distinguish them from flight search sessions.
...
| 400 | Search Already Initiated | A search has already been started for this session | ORX_SEARCH_ALREADY_STARTED |
| 400 | Invalid Session Type | The session ID does not correspond to a hotel search session | ORX_INVALID_SESSION_TYPE |
...
> 🛎 Poll this endpoint regularly to track search progress. The overall status becomes `COMPLETED`, `ERRORED` or `TIMED_OUT` when all streams finish processing.
...
| 400 | [Search Not Initiated] | No search has been started for this session | ORX_SEARCH_NOT_STARTED |
