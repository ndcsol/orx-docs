...
> 🛎 The session returned will be of flow `HOTEL_BOOKING`. This session is a long-lived one that will expire after the check-out date of the hotel reservation. You can see the details of the session using the [Get Session](/versions/v5/session#accessing-session-information) endpoint.
