...
- `diners`: Diners Club payment card
- `americanExpress`: American Express payment card
...
- `code`: ([Payment Card Type](#payment-card-type)) The [TokenEx cardType](https://docs.tokenex.com/docs/events#tokenize).
...
- `accepted_payment_cards`: (`Array<`[Payment Card Type](#payment-card-type)`>?`) List of accepted payment card types for this hotel rate
...
- `REFUND_OR_RETAIN`: Partial refund with some amounts retained
- `FORFEIT`: No refund available (full forfeiture)
- `RESERVATION`: No charges have been made, so no refund applicable
- `UNKNOWN`: Refund policy cannot be determined
...
- `REFUND`: Amount refunded to the original payment method
- `RETAIN`: Amount retained as credit or voucher for future use
...
- `type`: ([Refund Item Type](#refund-item-type)) Whether the amount is refunded or retained
- `amount`: (`number`) Refund or retention amount
- `currency`: (`string`) Currency code for the amount
- `expiration`: (`string?`) Expiration date for retained credits
...
- `type`: ([Refund Type](#refund-type)) Overall refund policy type
- `items`: (`Array<`[Refund Item](#refund-item)`>`) Detailed breakdown of refund items
- `vendor`: (`string?`) Vendor handling the refund
...
- `locator`: (`string`) Booking locator or confirmation number
- `refund`: ([Hotel Cancel Refund](#hotel-cancel-refund)) Detailed refund information for the cancellation check
...
- `locator`: (`string`) Booking locator or confirmation number
- `refund`: ([Hotel Cancel Refund](#hotel-cancel-refund)) Detailed refund information for the completed cancellation
