...
- `flow`: (`FLIGHT` | `FLIGHT_BOOKING` | `HOTEL`) The type of the session. `FLIGHT_BOOKING` indicates the session is used to retrieve information about an existing booking. `HOTEL` indicates a hotel search session.
...
```ts
interface Session {
  flow: "FLIGHT" | "FLIGHT_BOOKING" | "HOTEL";
  public: PublicID;
  expireAt: string;
  createdAt: string;
}
...
Represents comprehensive hotel information with all available details (extends [Hotel Information Preview](#hotel-information-preview)).

### Sub Properties

Inherits all properties from [Hotel Information Preview](#hotel-information-preview):
...
```ts
type HotelInformation = HotelInformationPreview;
```

> 🛎 Hotel Information is currently identical to Hotel Information Preview but is defined separately to allow for future extensions without breaking existing references.
...
- `hotel_information`: ([Hotel Information](#hotel-information)) Basic hotel information (inherited from availability)
