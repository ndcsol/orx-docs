...
### Hotel Events
- `HOTEL_BOOKING_ISSUED`: Triggered when a hotel booking is successfully confirmed.
- `HOTEL_BOOKING_CANCELLED`: Triggered when a hotel booking is cancelled.
...
- `eventType`: The event that occurred - can be any of the flight or hotel event types listed above.
- `session`: ([Session](/versions/v5/glossary#session)) The session object associated with the event.    
